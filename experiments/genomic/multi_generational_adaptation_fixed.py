#!/usr/bin/env python3
"""
Multi-Generational Adaptation Experiment - Fixed Version
...
class MultiGenerationalAdaptationExperiment:
                ...
                if response.status_code == 200:
                    ...
                    if data.get("success"):
                        ...
                        results["evolutionary_results"].append({
                            ...
                            "environment_zone": sensor["environment_zone"],
                            "evolutionary_pressure": evolutionary_pressure,
    ...
    def select_next_generation(self, evolutionary_results: List[Dict[str, Any]], 
        ...
        for i, organism_result in enumerate(selected):
            # Create offspring
            offspring_count = 1 if i < selected_count // 2 else 2  # Top performers reproduce more
            
            for j in range(offspring_count):
                offspring_id = f"gen{organism_result['generation']+1}_offspring_{i+1:03d}_{j+1}"
                
                # Inherit traits with possible mutations
                inherited_traits = organism_result["adaptation_traits"].copy()
                ...
                new_position = [
                    organism_result["position"][0] + random.uniform(-2, 2),
                    organism_result["position"][1] + random.uniform(-2, 2),
                    organism_result["position"][2] + random.uniform(-1, 1)
                ]
                
                offspring = {
                    "organism_id": offspring_id,
                    "generation": organism_result["generation"] + 1,
                    "position": new_position,
                    "sensitivity": max(0.3, min(1.0, organism_result["biasing_result"]["fidelity_score"] + random.uniform(-0.1, 0.1))),
                    "environment_zone": organism_result["environment_zone"],
                    "adaptation_traits": inherited_traits,
                    "fitness_score": 0.0,
                    "parent_ids": [organism_result["organism_id"]],
                    "genomic_lineage": organism_result.get("genomic_lineage", []) + [organism_result["organism_id"]]
                }
        ...
        while len(next_generation) < population_size:
            immigrant_id = f"gen{selected[0]['generation']+1}_immigrant_{len(next_generation)+1:03d}"
            immigrant = {
