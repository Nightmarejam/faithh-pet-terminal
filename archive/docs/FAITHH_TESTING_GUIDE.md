...
## ⚙️ Automated Quick Checks

Run these before a deeper manual session:

```bash
# Smoke test critical endpoints (health, status, ping, PULSE, filesystem capabilities)
FAITHH_BACKEND_URL=http://localhost:5557 ./venv/bin/python scripts/faithh_smoke_test.py

# Optional: include filesystem list (requires FAITHH_FILESYSTEM_TOKEN)
FAITHH_FILESYSTEM_TOKEN=... ./venv/bin/python scripts/faithh_smoke_test.py --include-filesystem

# Basic load/latency test (ping fastpath by default)
./venv/bin/python scripts/faithh_load_test.py --requests 10 --concurrency 2 --message "ping"
```
