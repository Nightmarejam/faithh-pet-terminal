#!/usr/bin/env python3
"""
Complete ALIFE Phase 2 Data Collection - Simple Approach

Target: Complete remaining 19 samples to reach 50 total
Current: 31/50 samples (62% complete)
Goal: 50/50 samples (100% complete)

This script uses direct FAITHH backend API calls instead of ALIFE observer.
"""
...
def query_faithh(message, model="qwen25-grounded:latest"):
    """Query FAITHH backend API"""
...
def complete_phase2_data_collection():
    ...
    # Check FAITHH backend availability
    print("🔄 Checking FAITHH backend availability...")
    health_check = requests.get("http://localhost:5557/health", timeout=10)
    
    if health_check.status_code != 200:
        print("❌ FAITHH backend not available. Please start the backend first:")
        print("   ./restart_backend.sh")
        return 0
    
    print("✅ FAITHH backend is available")
    print()
    ...
    for i, query in enumerate(queries, 1):
        ...
        if response.get('success'):
            ...
            success_count += 1
            print(f"   ✅ Success ({response.get('model_used', 'unknown')})")
        else:
