...
class AutomatedHealthCheck:
    ...
    def __init__(self):
        ...
        self.chromadb_url = "http://servicebox.taileb8c60.ts.net:8000"
    ...
    def check_backend_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                # Canonical ecosystem snapshot (includes faithh_status == former /api/status body)
                plc_response = requests.get(f"{self.backend_url}/api/plc/state", timeout=10)

                if plc_response.status_code == 200:
                    plc_data = plc_response.json()
                    fs = plc_data.get("faithh_status") or {}
                    return {
                        "status": "healthy",
                        "response_time": response_time,
                        "plc_state": plc_data,
                        "backend_status": fs,
                        "uptime": fs.get("uptime", plc_data.get("uptime", "unknown")),
                        "active_connections": fs.get("active_connections", 0),
                    }
                else:
                    return {
                        "status": "warning",
                        "response_time": response_time,
                        "warning": "PLC state endpoint not responding",
                    }
            else:
                return {
                    "status": "error",
            ...
            if response.status_code == 200:
                ...
                if collections_response.status_code == 200:
                    ...
                    return {
                        "status": "warning",
                        "response_time": response_time,
                        "warning": "Collections endpoint not responding"
                    }
            else:
                return {
                    "status": "error",
    ...
    def check_service_dependencies(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if docker_result.returncode != 0:
                return {
                    "status": "error",
                    "error": "Docker Compose not responding"
                }
