...
MODEL="${MODEL:-llama31-faithh:latest}"
...
if echo "$health_response" | jq -e '.status == "healthy"' > /dev/null 2>&1; then
    ...
    echo "  cd ~/ai-stack && ./restart_backend.sh"
    exit 1
fi
echo ""
...
test_endpoint \
    ...
    "$BASE_URL/api/chat" \
    '{"message": "Hello FAITHH", "model": "'"$MODEL"'", "use_rag": false}' \
    '"success":\s*true'
...
test_endpoint \
    ...
    "$BASE_URL/api/chat" \
    '{"message": "What is the FAITHH project?", "model": "'"$MODEL"'", "use_rag": true}' \
    '"success":\s*true'
...
measure_performance \
    "Chat with RAG Speed" \
    "$BASE_URL/api/chat" \
    '{"message": "Quick test", "model": "'"$MODEL"'", "use_rag": true}'
