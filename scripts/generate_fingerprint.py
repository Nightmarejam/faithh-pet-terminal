...
def check_chromadb_health() -> dict:
    ...
    try:
        import requests
        r = requests.get("http://192.158.1.243:8000/api/v2/heartbeat", timeout=5)
        if r.ok:
            return {"status": "healthy", "host": "192.158.1.243", "port": 8000}
        return {"status": "unhealthy", "host": "192.158.1.243", "port": 8000, "error": f"HTTP {r.status_code}"}
    except Exception as e:
        return {"status": "unreachable", "host": "192.158.1.243", "port": 8000, "error": str(e)}
