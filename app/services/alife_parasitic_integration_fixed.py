"""
Alife Parasitic Integration Service - Fixed
Phase 3B: Connect parasitic feeding to existing Alife data
Fixed infinite loop issue in signature mapping
"""
...
class AlifeParasiticIntegration:
    ...
    def calculate_parasitic_signature(self, event: Dict[str, Any]) -> ParasiticSignature:
        """Calculate parasitic signature for an Alife event - FIXED"""
        try:
    ...
    def _calculate_parasitic_efficiency(self, event: Dict[str, Any]) -> float:
        """Calculate parasitic efficiency based on event characteristics - FIXED"""
        try:
            ...
            # Adjust based on event complexity - FIXED infinite loop prevention
            event_str = str(event)
            complexity_factor = min(len(event_str) / 1000, 1.0)  # Complexity up to 1.0
            
            # Energy-based adjustment
            metadata = event.get('metadata', {})
            if metadata and 'agent_energy' in metadata:
                energy_level = metadata.get('agent_energy', 50)
                energy_factor = min(energy_level / 100, 1.0)
            else:
                energy_factor = 0.5
            
            # Combined efficiency - FIXED calculation
    ...
    def _calculate_moon_correlation(self, event: Dict[str, Any]) -> float:
        """Calculate correlation with Moon damping effects - FIXED"""
        try:
            ...
            if 'agent_energy' in metadata:
                energy = metadata.get('agent_energy', 50)
                if energy > 50:  # High energy indicates stability
                    stability_score += 0.3
            
            # Pattern stability (repetitive behaviors)
            if 'genome_readable' in metadata:
                genome = metadata.get('genome_readable', '')
                if len(genome) > 20:  # Complex genome indicates stability
                    stability_score += 0.2
    ...
    def _estimate_impedance_gradient(self, event: Dict[str, Any]) -> float:
        """Estimate impedance gradient from event complexity - FIXED"""
        try:
            # Calculate complexity-based gradient - FIXED
            event_str = str(event)
            ...
            if 'agent_energy' in metadata:
                energy = metadata.get('agent_energy', 50)
                if energy > 100:  # High energy increases gradient
                    gradient += 2
    ...
    def _calculate_cognitive_complexity(self, event: Dict[str, Any]) -> float:
        """Calculate cognitive complexity from event data - FIXED"""
        try:
            event_type = event.get('domain', event.get('type', 'unknown'))
            ...
            if 'agent_energy' in metadata:
                energy = metadata.get('agent_energy', 50)
                if energy > 100:  # High energy suggests complex cognition
                    base_complexity += 0.1
    ...
    def _calculate_fusion_potential(self, event: Dict[str, Any]) -> float:
        """Calculate fusion-like process potential - FIXED"""
        try:
            # Combine parasitic efficiency and cognitive complexity
            signature = self.calculate_parasitic_signature(event)
            
            # Energy availability from metadata
            metadata = event.get('metadata', {})
            energy_available = metadata.get('agent_energy', 50) / 100
            
            fusion_potential = signature.parasitic_efficiency * signature.cognitive_complexity * energy_available
    ...
    def _determine_earth_mars_analog(self, event: Dict[str, Any]) -> str:
        """Determine if event is Earth-like or Mars-like - FIXED"""
        try:
            signature = self.calculate_parasitic_signature(event)
    ...
    def map_parasitic_signatures(self) -> Dict[str, Any]:
        """Map parasitic signatures to all Alife events - FIXED"""
        try:
            if not self.alife_data:
                return {"error": "No Alife data loaded"}
