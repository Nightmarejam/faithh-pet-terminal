...
| `scripts/system_pulse.py` (disk I/O, network vital, Chroma count, RAM vs `.wslconfig`) | Same Chroma env as above; optional `rich` for layout; `PULSE_DSM_HOST` (default `nas.taileb8c60.ts.net`) for DSM ping; checks `/mnt/nas-staging` for NFS | Pulse dashboard; `CHROMA_COLLECTION`; `RAW_DATA_STAGING` not read here |
