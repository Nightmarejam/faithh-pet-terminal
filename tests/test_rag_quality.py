...
TEST_QUERIES = [
    {
        ...
        "expected_sources": ["SYSTEM_OVERVIEW", "README", "CONTEXT", "AGENTS"],
    ...
    },
    {
        ...
        "expected_sources": ["README", "QUICKSTART", "SYSTEM_OVERVIEW", "SYSTEMS_MAP", "guide", "architecture"],
    ...
    },
    {
        ...
        "expected_sources": ["architecture", "SYSTEM_OVERVIEW", "BACKEND_API", "INFRASTRUCTURE"],
    ...
    },
    {
        ...
        "expected_sources": ["LIFE_MAP", "CONTEXT", "HARDWARE", "life_map"],
    ...
    },
    {
        ...
        "expected_sources": ["SYSTEM_OVERVIEW", "MODEL_INVENTORY", "BACKEND_API", "architecture"],
    ...
    },
    {
        ...
        "expected_sources": ["CONSTELLA", "constella", "HARMONY", "harmony"],
...
class RAGQualityTester:
    ...
    def _merge_tiered_results(self, tier1, tier2, n_results):
        """Merge project_docs (tier1) and full-collection (tier2) results, deduped."""
        merged = {'documents': [[]], 'metadatas': [[]], 'distances': [[]], 'ids': [[]]}
        seen = set()

        for source in [tier1, tier2]:
            if not source or not source.get('ids') or not source['ids'][0]:
                continue
            docs = source['documents'][0]
            metas = source.get('metadatas', [[]])[0] or [{}] * len(docs)
            dists = source.get('distances', [[]])[0] or [0.5] * len(docs)
            ids = source['ids'][0]
        ...
        # Trim to n_results (project_docs already first due to ordering)
        for key in merged:
            merged[key][0] = merged[key][0][:n_results]
        return merged
    ...
    def run_test(self, query_data: Dict) -> Dict[str, Any]:
        ...
        # Two-tier query: project_docs first (authoritative), then full collection
        try:
            # Tier 1: Query project_docs specifically
            try:
            ...
            # Tier 2: Query full collection
            tier2 = self.collection.query(
                query_texts=[query],
                n_results=self.n_results * 2
            )

            # Merge: deduplicate, project_docs first
            results = self._merge_tiered_results(tier1, tier2, self.n_results)
