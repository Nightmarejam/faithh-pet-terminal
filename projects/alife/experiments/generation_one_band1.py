#!/usr/bin/env python3
"""
Generation one — Band 1 with tighter resource pressure.
Acquisition: 0.5-1.5 (was 0.5-2.0)
Loss: 0.4-1.6 (was 0.3-1.5)
Expected net per tick: ~0 (tighter caps vs gen zero’s +0.35); should produce real filtering.
"""
...
# Generation one parameters — tighter than generation zero
NUM_AGENTS = 50
NUM_TICKS = 10
STARTING_RESOURCES = 10.0
ACQUISITION_MIN = 0.5
ACQUISITION_MAX = 1.5  # tighter — was 2.0
LOSS_MIN = 0.4  # tighter — was 0.3
LOSS_MAX = 1.6  # tighter — was 1.5
...
def run():
    ...
    for tick in range(1, NUM_TICKS + 1):
        for agent in agents:
            ...
            all_ids.append(f"gen1_{agent['id']}_tick{tick:02d}")
    ...
    top10 = sorted_agents[:5]
    bottom10 = sorted_agents[-5:]
    ...
    report_path = REPORT_DIR / f"generation_one_band1_{ts}.json"
