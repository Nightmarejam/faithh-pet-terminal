#!/usr/bin/env python3
"""
Generation three — Band 1, Options A+B combined.

Starting resources: 3.0 (Option B — was 10.0 in gen two).
Option A — same negative drift: acquisition 0.3–1.2 (mean 0.75), loss 0.5–1.5 (mean 1.0),
net ≈ −0.25 per tick in expectation.

With start 3.0, mean time to depletion is on the order of ~12 ticks; variance produces
real filtering within 10 ticks (some agents hit zero early, others survive full horizon).
"""
...
# Generation three — Option A drift + Option B low starting pool
NUM_AGENTS = 50
NUM_TICKS = 10
...
def run():
    ...
    for tick in range(1, NUM_TICKS + 1):
        for agent in agents:
            ...
            all_ids.append(f"gen3_{agent['id']}_tick{tick:02d}")
    ...
    report_path = REPORT_DIR / f"generation_three_band1_{ts}.json"
