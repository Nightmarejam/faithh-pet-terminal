#!/usr/bin/env python3
"""
Band 2 generation 4 — quality-weighted Band 3 selection (band3_score gate).

Same mechanics as gen 3; Band 3 uses mutual_benefit / false_coop / net_resource score.
Symmetric (A) vs asymmetric (B) signal interpretation.
...
def trust_network_stability(agents: dict[str, dict[str, Any]], agent_ids: list[str]) -> float:
    """1.0 / (1.0 + variance of all trust scores); higher = more stable (lower variance)."""
    vals: list[float] = []
...
def run_population(
    ...
    # Band 3 candidates + summary stats (quality-weighted Band 3 gate)
    band3: list[str] = []
    ...
    for aid, ag in agents.items():
        ...
        if band3_score >= 2.5 and ag["defection_count"] == 0:
            band3.append(aid)
...
def main() -> int:
    ...
    out_path = REPORT_DIR / f"band2_generation4_{stamp}.json"
