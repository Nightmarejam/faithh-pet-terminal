#!/usr/bin/env python3
"""
Band 2 generation 7 — confirmation variant (0.2 vs 0.4 noise).
...
def interpret_signal(true_signal: float, population: str, noise_amp: float) -> float:
    if population == "A":
        return true_signal
...
def run_population(
    ...
    for tick in range(1, NUM_TICKS + 1):
        ...
        for a, b in edges:
            ...
            interp_b_for_a = interpret_signal(true_sb, population, noise_amp)
            interp_a_for_b = interpret_signal(true_sa, population, noise_amp)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.2,
        help="Population B additive signal noise half-range. 0.2 means ±0.2.",
    )
    parser.add_argument(
        "--label",
        default="",
...
def main() -> int:
    ...
    print(f"Connecting to ChromaDB... (noise_amp={noise_amp})", flush=True)
    ...
    summary_a, _ = run_population("A", col, run_id, noise_amp)
    summary_b, _ = run_population("B", col, run_id, noise_amp)
    ...
    out_path = REPORT_DIR / f"band2_generation7_n{noise_tag}{label_part}_{stamp}.json"
