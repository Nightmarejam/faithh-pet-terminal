#!/usr/bin/env python3
"""
Band 2 generation 8 — calibrated uncertainty vs manufactured asymmetry.

Same mechanics as generation 5, but Population B interpretation noise amplitude
is parameterized via --noise-amp for stochastic resonance sweeps.
"""
...
NUM_AGENTS = 30
NUM_TICKS = 15
COOPERATION_DECISION_THRESHOLD = 0.65
START_RESOURCES = 5.0
GENERATION = 8
EXPERIMENT = "band2_cooperation_gen8_bias"
...
def interpret_signal(true_signal: float, population: str, noise_amp: float, bias_shift: float) -> float:
    """A: calibrated uncertainty (symmetric noise), B: manufactured asymmetry (same noise + directional bias)."""
    noise = (oracle_random() - 0.5) * (2.0 * noise_amp)
    if population == "A":
        interpreted = true_signal + noise
    else:
        interpreted = true_signal + noise + bias_shift
    return max(0.0, min(1.0, interpreted))
...
def make_agent(agent_id: str, population: str) -> dict[str, Any]:
    return {
        ...
        "mutual_benefit_events": 0,
        "false_cooperation_events": 0,
        "net_resource_from_cooperation": 0.0,
        "band3_score": 0.0,
    }
...
def trust_network_stability(agents: dict[str, dict[str, Any]], agent_ids: list[str]) -> float:
    """1 / (1 + variance of trust scores). Higher means more stable."""
    vals: list[float] = []
    for aid in agent_ids:
        vals.extend(agents[aid]["trust_scores"].values())
    if not vals:
        return 1.0
...
def run_population(
    ...
    for tick in range(1, NUM_TICKS + 1):
        ...
        for a, b in edges:
            ...
            interp_b_for_a = interpret_signal(true_sb, population, noise_amp, bias_shift)
            interp_a_for_b = interpret_signal(true_sa, population, noise_amp, bias_shift)
            ...
            if a_coops and b_coops:
                ...
                gain = 0.2 * min(true_sa, true_sb)
                cost_a = true_sa * 0.1
                cost_b = true_sb * 0.1
                net_a = gain - cost_a
                net_b = gain - cost_b
                if net_a > 0 and net_b > 0:
                    mutual_benefit_events += 1
                    agent_a["mutual_benefit_events"] += 1
                    agent_b["mutual_benefit_events"] += 1
                else:
                    false_cooperation_events += 1
                    agent_a["false_cooperation_events"] += 1
                    agent_b["false_cooperation_events"] += 1
                agent_a["net_resource_from_cooperation"] += net_a
                agent_b["net_resource_from_cooperation"] += net_b
        ...
        for ag in agents.values():
            ...
            cooperation_rate = cc / max(1, cc + dc)
    ...
    accs: list[float] = []
    band3_scores: list[float] = []
    for aid, ag in agents.items():
        fitnesses.append(ag["fitness"])
        accs.append(ag["signal_accuracy"])
        mb = ag["mutual_benefit_events"]
        fc = ag["false_cooperation_events"]
        mutual_benefit_rate = mb / max(1, mb + fc)
        false_coop_rate = fc / max(1, mb + fc)
        net_resource = ag["net_resource_from_cooperation"]
        band3_score = (mutual_benefit_rate * 2.0) + (1.0 - false_coop_rate) + (net_resource / 10.0)
        ag["band3_score"] = round(band3_score, 3)
        band3_scores.append(ag["band3_score"])
        if band3_score >= 1.90 and ag["defection_count"] == 0:
            band3.append(aid)
    ...
    nets = [agents[aid]["net_resource_from_cooperation"] for aid in agent_ids]
    mean_net = sum(nets) / len(nets) if nets else 0.0
    t_stab = trust_network_stability(agents, agent_ids)
    ...
    summary = {
        ...
        "mutual_benefit_events": mutual_benefit_events,
        "false_cooperation_events": false_cooperation_events,
        "mutual_benefit_rate": mutual_benefit_events / max(1, cooperation_events),
        "false_cooperation_rate": false_cooperation_events / max(1, cooperation_events),
        "mean_net_resource_from_cooperation": mean_net,
        "trust_network_stability": t_stab,
        "mean_band3_score": sum(band3_scores) / len(band3_scores) if band3_scores else 0.0,
    }
    return summary, band3


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Band 2 generation 6 with configurable B-noise.")
    parser.add_argument(
        "--noise-amp",
        type=float,
        default=0.2,
        help="Shared additive signal noise half-range for both populations.",
    )
    parser.add_argument(
        "--label",
        default="",
        help="Optional run label for filenames/ids.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=-0.12,
        help="Directional shift applied only to Population B interpreted signals.",
    )
...
def main() -> int:
    args = parse_args()
    noise_amp = max(0.0, args.noise_amp)
    bias_shift = args.bias_shift
    noise_tag = str(round(noise_amp, 3)).replace(".", "p")
    label = args.label.strip()
    label_part = f"_{label}" if label else ""

    REPORT_DIR.mkdir(parents=True, exist_ok=True)

    print(f"Connecting to ChromaDB... (noise_amp={noise_amp}, bias_shift={bias_shift})", flush=True)
    ...
    run_id = f"n{noise_tag}{label_part}_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}"
    summary_a, _ = run_population("A", col, run_id, noise_amp, bias_shift)
    summary_b, _ = run_population("B", col, run_id, noise_amp, bias_shift)

    alife_after = col.count()

    mean_band3_a = summary_a["mean_band3_score"]
    mean_band3_b = summary_b["mean_band3_score"]
    band3_score_gap = mean_band3_a - mean_band3_b

    hypothesis = {
        "A_higher_mutual_benefit_rate": summary_a["mutual_benefit_rate"] > summary_b["mutual_benefit_rate"],
        "A_lower_false_cooperation_rate": summary_a["false_cooperation_rate"] < summary_b["false_cooperation_rate"],
        "A_more_band3_candidates": len(summary_a["band3_candidates"]) > len(summary_b["band3_candidates"]),
        "A_higher_trust_stability": summary_a["trust_network_stability"] > summary_b["trust_network_stability"],
        "A_better_net_resource": summary_a["mean_net_resource_from_cooperation"]
        > summary_b["mean_net_resource_from_cooperation"],
        "A_higher_mean_band3_score": mean_band3_a > mean_band3_b,
        "hypothesis_supported": False,
    }
    hypothesis["hypothesis_supported"] = all(
        [
            hypothesis["A_higher_mutual_benefit_rate"],
            hypothesis["A_lower_false_cooperation_rate"],
            hypothesis["A_more_band3_candidates"],
            hypothesis["A_higher_trust_stability"],
            hypothesis["A_better_net_resource"],
            hypothesis["A_higher_mean_band3_score"],
        ]
    )
    ...
    out = {
        ...
        "population_A_mode": "calibrated_uncertainty",
        "population_B_mode": "manufactured_asymmetry",
        "randomness_source": "oracle_urandom",
        "population_A": summary_a,
        "population_B": summary_b,
        "band3_score_gap": band3_score_gap,
    ...
    out_path = REPORT_DIR / f"band2_generation8_n{noise_tag}{label_part}_{stamp}.json"
