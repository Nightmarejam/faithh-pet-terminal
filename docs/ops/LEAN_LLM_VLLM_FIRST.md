...
2. **`configs/model_config.yaml`:** set `providers.local_webui.base_url` to your `/v1` URL (often `http://127.0.0.1:8000/v1`). Set `model` to an **`id`** from that JSON.
...
Install Ollama on **faithh**; add **`ollama`** back into `routes.auto` in `model_config.yaml` if you want fallback. Keep **`FAITHH_FORCE_LOCAL=0`** unless you intentionally want Ollama-only routing.
...
**Docker bridges:** what matters is **bind** (`0.0.0.0:8000` for Chroma so faithh can reach Gen8 LAN IP). Check: `docker ps`, `ss -tlnp`.
