...
- Gen8 LAN: `192.158.1.243` (`eno2`) per `network_baseline_2026-04-02.md` and live Gen8 `ss` output.
...
| **`192.158.1.1`** | SSH **reachable** — server offered host key; authentication **`Permission denied (publickey,keyboard-interactive)`** for `root` |
...
- **Likely gateway / UniFi appliance:** `192.158.1.1` (same /24 as Gen8).
- **SSH:** **Reachable (yes)** — TCP/22 responds; **Authenticated access (no)** without UniFi-configured SSH key or password for `root`.
- **Next step:** In UniFi: **Settings → System → Device SSH** — enable SSH, set credentials/keys, then add `Host udm` in `~/.ssh/config` with `HostName 192.158.1.1` and the correct user/key.
